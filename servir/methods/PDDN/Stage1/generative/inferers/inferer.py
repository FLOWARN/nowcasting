...
class LatentDiffusionInferer(DiffusionInferer):
    ...
    def __call__(
        self,
        inputs: torch.Tensor,
        autoencoder_model_radar: Callable[..., torch.Tensor],
        autoencoder_model_wrf: Callable[..., torch.Tensor],
        diffusion_model: Callable[..., torch.Tensor],
        noise: torch.Tensor,
        timesteps: torch.Tensor,
        condition: torch.Tensor | None = None,
...
#            )
        ...
        with torch.no_grad():
            conditioning_radar = autoencoder_model_radar.encode(conditioning[:, :1]) * self.scale_factor
            conditioning_wrf = autoencoder_model_wrf.encode(conditioning[:, 1:]) * self.scale_factor

        conditioning = torch.cat((conditioning_radar, conditioning_wrf), 1)
...
#            decode = partial(autoencoder_model.decode_stage_2_outputs, seg=seg)
        image = autoencoder_model_radar.decode(latent / self.scale_factor)
...
#        else:
    ...
    def get_likelihood(
        ...
        latents = autoencoder_model.encode(inputs) * self.scale_factor
