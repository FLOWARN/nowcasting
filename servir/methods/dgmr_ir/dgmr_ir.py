...
from servir.methods.dgmr_ir.common import ContextConditioningStack, LatentConditioningStack
from servir.methods.dgmr_ir.discriminators import Discriminator
from servir.methods.dgmr_ir.generators import Generator, Sampler
from servir.methods.dgmr_ir.hub import NowcastingModelHubMixin
from servir.methods.dgmr_ir.losses import (
    GridCellLoss,
    NowcastingLoss,
...
class DGMR_IR(pl.LightningModule, NowcastingModelHubMixin):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for _ in range(2):
            d_opt.zero_grad()
            predictions = self(images, ir_images)
        ...
        # Optimize Generator #
        ######################
        predictions = [self(images, ir_images) for _ in range(self.generation_steps)]
        ...
        if self.visualize:
            # generate images
            generated_images = self(images, ir_images)
