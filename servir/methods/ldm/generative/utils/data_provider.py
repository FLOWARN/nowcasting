...
# class IMERGDataModuleLatentDim(torch.utils.data.Dataset):
#     def __init__(self, imerg_filename=None, ir_filename=None, forecast_steps=2, history_steps=2,
#                  normalize_data=False, image_shape=(64, 64), production_mode=False, mode=None):
#         super().__init__()
...
#         # Dimensions: (samples, channels, time, height, width)
#         # Using 32 channels and 2 time steps for latent dim as in your example
#         channels = 32
#         time_steps = 2
        
#         def generate_spatiotemporal_pattern(n_samples, c, t, h, w):
#             x = torch.linspace(0, 2 * math.pi, w).view(1, 1, 1, 1, w).repeat(n_samples, c, t, h, 1)
#             y = torch.linspace(0, 2 * math.pi, h).view(1, 1, 1, h, 1).repeat(n_samples, c, t, 1, w)
#             time = torch.linspace(0, 2 * math.pi, t).view(1, 1, t, 1, 1).repeat(n_samples, c, 1, h, w)
#             channel = torch.linspace(0, 2 * math.pi, c).view(1, c, 1, 1, 1).repeat(n_samples, 1, t, h, w)
            
#             # Combine sine waves with different frequencies and phases
#             pattern = torch.sin(x + channel + time) * torch.cos(y + channel - time)
...
#         self.input_precipitation = generate_spatiotemporal_pattern(num_samples, channels, time_steps, self.img_height, self.img_width)
#         self.output_precipitation = generate_spatiotemporal_pattern(num_samples, channels, time_steps, self.img_height, self.img_width)
#         self.input_IR = generate_spatiotemporal_pattern(num_samples, channels, time_steps, self.img_height, self.img_width)
        
#         # Concatenate precipitation and IR along channel dim (dim=1)
#         self.condition = torch.cat((self.input_precipitation, self.input_IR), dim=1)

#     def __getitem__(self, idx):
#         return self.condition[idx], self.output_precipitation[idx]

#     def __len__(self):
#         return self.input_precipitation.shape[0]
...
class IMERGDataModuleLatentDim(Dataset):
    ...
    def __init__(self, imerg_filename, ir_filename, forecast_steps, history_steps, normalize_data=False,
        ...
        # self.input_precipitation = torch.rand(num_samples, 32, 2,  64, 64)
        # self.output_precipitation = torch.rand(num_samples, 32, 2, 64, 64)
        # self.input_IR = torch.rand(num_samples,  32, 2, 64, 64)
        
        
        self.input_precipitation = torch.ones(num_samples, 32, 2, 64, 64)
        self.output_precipitation = torch.ones(num_samples, 32, 2, 64, 64)
        self.input_IR = torch.ones(num_samples, 32, 2, 64, 64)
