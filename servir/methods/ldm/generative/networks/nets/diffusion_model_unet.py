...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.ff = MLPBlock(hidden_size=num_channels+64, mlp_dim=num_channels * 4, act="GEGLU", dropout_rate=dropout)
        ...
        self.norm2 = nn.LayerNorm(num_channels+64)
        self.norm3 = nn.LayerNorm(num_channels+64)
...
#        print(context.shape)
        ...
        x = torch.concat((self.norm1(x),context), axis=-1)
        
        x = self.attn1(self.norm2(x)) + x
        
        

        # 2. Cross-Attention
        x = self.attn2(self.norm2(x), context=x) + x
...
#        self.proj_in_condition = Convolution(
#            spatial_dims=spatial_dims,
#            in_channels=3,
#            out_channels=inner_dim,
#            strides=1,
...
#        print(context.shape)
        ...
        if x.shape[-1]==64:
            ...
            context = F.avg_pool3d(context, (1,2,2))
        elif x.shape[-1]==16:
            context = F.avg_pool3d(context, (1,2,2))
            context = F.avg_pool3d(context, (1,2,2))
        elif x.shape[-1]==8:
        ...
        batch_condtion, channel_condition, height_condition, width_condition, depth_condition = context.shape
#        context = self.proj_in_condition(context)
        inner_dim_condition = context.shape[1]
        context = context.permute(0, 2, 3, 4, 1).reshape(batch_condtion, height_condition * width_condition * depth_condition, inner_dim_condition)
...
#        x = torch.concat((self.norm1(x),y), axis=-1)
        ...
        if self.spatial_dims == 3:
            x = x.reshape(batch, height, width, depth, inner_dim+64).permute(0, 4, 1, 2, 3).contiguous()
