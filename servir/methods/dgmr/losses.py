...
def FSS(pred, gt, device, threshold=8., n=8):
    ...
    pred = torch.where(pred<= threshold, 0.0,1.0)
    gt = torch.where(gt<= threshold, 0.0,1.0)

    num_samples = pred.shape[1]

    convolution_matrix = (torch.ones((1,1,n,n))/(n*n)).to(device)
    FSS = 0.0
    if n >=1:
        # compute the FSS score for each image with weight given equally among all images
        for i in range(num_samples):
            pred_modified = torch.nn.functional.conv2d(pred[:,i, :, :,:],convolution_matrix)
            gt_modified = torch.nn.functional.conv2d(gt[:,i, :, :,:],convolution_matrix)
            FSS += 2*(torch.multiply(pred_modified,gt_modified).sum())/(torch.square(pred_modified).sum() + torch.square(gt_modified).sum())
    return FSS
