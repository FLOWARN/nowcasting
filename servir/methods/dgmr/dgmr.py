...
from servir.methods.dgmr.common import ContextConditioningStack, LatentConditioningStack
from servir.methods.dgmr.discriminators import Discriminator
from servir.methods.dgmr.generators import Generator, Sampler
from servir.methods.dgmr.hub import NowcastingModelHubMixin
from servir.methods.dgmr.losses import (
    GridCellLoss,
    NowcastingLoss,
...
class DGMR(pl.LightningModule, NowcastingModelHubMixin):
    ...
    def predict_ensemble(self, x, n_ens_members = 10, is_numpy=False):
        predictions = [self.generator(x) for _ in range(n_ens_members)]
        if is_numpy:
    ...
    def training_step(self, batch, batch_idx):
        images, images_ir, future_images = batch
        ...
        ensemble_prediction = self.predict_ensemble(images, n_ens_members=20, is_numpy = True)
        ...
        for item, value in log_dictionary.items():
            self.log(item, value, prog_bar=True, logger=True, on_step=True, on_epoch=True, sync_dist=True)
        
        
        # self.log("discriminator_loss", discriminator_loss, prog_bar=True, logger=True, on_step=True, on_epoch=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        ##########################
        # Forward propagate discriminator #
        ##########################
        # Two discriminator steps per generator step
        for _ in range(2):
            predictions = self(images)
